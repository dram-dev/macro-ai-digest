...
def test_reddit() -> bool:
    """Test the active Reddit path. Default = public JSON. PRAW only if flag set."""
    use_praw = os.getenv("REDDIT_USE_PRAW", "").lower() in ("1", "true", "yes")
    try:
        if use_praw:
            if not settings.reddit_client_id:
                line(FAIL, "reddit", "REDDIT_USE_PRAW=true but REDDIT_CLIENT_ID empty")
                return False
            import praw
            ...
            line(PASS, "reddit", f"PRAW r/investing top = '{post.title[:50]}...'")
            return True
        ...
        r = requests.get(
            ...
            params={"t": "day", "limit": 1, "raw_json": 1},
        ...
        if not children:
            line(WARN, "reddit", "JSON returned 0 posts — try again or check User-Agent")
            return False
        title = children[0].get("data", {}).get("title", "?")
        line(PASS, "reddit", f"JSON r/investing top = '{title[:50]}...'")
        return True
    except Exception as exc:  # noqa: BLE001
